...
export function scanPackage(dir, name) {
	...
	for (const file of files) {
		...
		runtimeSurface.childProcess += (text.match(/(?:^|[^.]\b)\bspawn\s*\(/g) ?? []).length;
...
/** Whether a path resolves to an existing directory (follows symlinks). */
...
export function discoverPluginPackages(roots) {
	...
	for (const root of roots) {
		...
		for (const entry of entries) {
			// link:-installed plugins appear as symlinks/junctions, so a
			// symlink is a candidate — but it must resolve to an EXISTING
			// directory, or it becomes a phantom SAFE entry (a dangling or
			// file-target link named like a plugin).
			if (!entry.isDirectory() && !entry.isSymbolicLink()) continue;
			const name = entry.name;
			const candidate = join(root, name);
			if (!statIsDirectory(candidate)) continue;
			if (name.startsWith("@")) {
				const scopeDir = candidate;
				let scoped;
				try {
				...
				for (const sub of scoped) {
					...
					if (!statIsDirectory(join(scopeDir, sub.name))) continue;
...
export function discoverOfficialPackages(roots) {
	...
	for (const root of roots) {
		...
		for (const entry of entries) {
			// same candidate rule as discoverPluginPackages: symlinks are
			// candidates (junction-installed plugins), but only if they
			// resolve to an existing directory.
			if (!entry.isDirectory() && !entry.isSymbolicLink()) continue;
			if (!statIsDirectory(join(root, entry.name))) continue;
			if (entry.name.startsWith("@")) {
...
export function runVet(roots, options = {}) {
	...
	const warnings = [
		"heuristic scan only: a clean result is not a security guarantee / 仅启发式扫描：干净结果不保证安全",
		"runtime-dynamic code (downloaded then eval'd, remote modules) is NOT in scope — static scan cannot see it / 运行时动态加载的代码不在静态扫描范围",
		"official @deepseek-ai/* packages are exempt by name / 官方 @deepseek-ai/* 包按名豁免",
		"false-positive class: security plugins that intentionally reference secret paths / exfil hosts in their own rules will score HIGH; add them to the allowlist / 已知误报类：安全插件自身规则引用密钥路径/外传域名会得高分，可用 allowlist 处理"
	];
	if (allowlist.size > 0) warnings.push(`${allowlist.size} package(s) allowlisted by config / ${allowlist.size} 个包已按配置白名单`);
