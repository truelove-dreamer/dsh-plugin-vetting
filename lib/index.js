 ...
 * /proc/<pid>/environ, ...). Log-only by default; set `enforce: true` to
 * BLOCK flagged spawns (a narrow, conservative gate — same-process hook,
 * not a kernel boundary; keep it off unless you accept the false-positive
 * risk, because a disabled guard is worse than none).
 *
 ...
 *       monitor: false        # enable the subprocess tripwire (log)
 *       enforce: false        # block flagged spawns (experimental gate)
...
/** Install the optional subprocess tripwire (log-only) or gate (enforce). */
function installMonitor(ctx, roots, enforce) {
	...
	ctx.effect(() => {
		subprocess.spawn = (spec) => {
			...
			if (FLAG_PATTERN.test(argv)) {
				...
				if (enforce === true) {
					throw new Error(`[plugin-vet] gate: blocked suspicious spawn (config enforce=true): ${detail}`);
				}
