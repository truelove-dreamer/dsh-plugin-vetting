 ...
 *
 * Optional plugin-tool gate (config `gate`, default "off"): when set to
 * "deny-unvetted", the tool pipeline refuses calls to tools registered
 * AFTER this plugin installs (i.e. plugin-provided tools) unless the tool
 * name is allowlisted. This is the tools-pipeline gate (`tools.guard`, the
 * same surface `tools/pre-execute` hooks) — it gates the MODEL-to-plugin
 * TOOL-CALL surface only, NOT plugin-internal code (child_process/fetch/
 * eval inside a plugin body never passes through the tools pipeline).
 * Name-based attribution is heuristic (scoped shadowing can spoof a
 * built-in name); default off because a disabled guard is worse than none.
 *
 ...
 *       gate: off             # "deny-unvetted" to gate plugin tool calls
 *       allowlistTools: [ ]   # tool names permitted even when unvetted
...
/** Bilingual risk tag: English grade plus Chinese label. */
const RISK_ZH = { SAFE: "安全", LOW: "低危", MEDIUM: "中危", HIGH: "高危", ALLOWED: "白名单" };

/** Render one package row into text. */
function renderPackage(p) {
	const lines = [];
	const tag = p.allowlisted === true ? "ALLOWED" : p.risk;
	const zh = RISK_ZH[tag] ?? tag;
	lines.push(`[${tag}/${zh}] ${p.name}@${p.version} (score ${p.score})${p.needsReview === true ? " [REVIEW 需人工确认: dynamic code execution 动态代码执行]" : ""}`);
	for (const f of p.findings.slice(0, 10)) {
		lines.push(`    - ${f.id}: ${f.note} ${f.noteZh !== undefined ? `/ ${f.noteZh}` : ""} @ ${f.file}:${f.line}`);
	}
	if (p.findings.length > 10) lines.push(`    ... and ${p.findings.length - 10} more`);
	if (p.findings.length === 0) lines.push("    (no malicious-pattern hits / 未发现恶意模式)");
	if (p.sloppyFindings !== undefined && p.sloppyFindings.length > 0) {
		lines.push(`    suggest narrowing (not suspicion) / 建议收窄权限（非可疑）:`);
	...
	const d = p.deps;
	if (d !== undefined) {
		const own = d.ownScripts?.length > 0 ? d.ownScripts.join(", ") : "none";
		lines.push(`    lifecycle scripts: ${own} | deps: ${d.declared} declared, ${d.nestedScanned} nested scanned, ${d.unchecked} unchecked`);
...
function renderReport(report) {
	const lines = [];
	lines.push(`# Plugin vet — ${report.scanned} third-party plugin(s) scanned / 已扫描 ${report.scanned} 个第三方插件`);
	lines.push(`safe=${report.summary.safe} low=${report.summary.low} medium=${report.summary.medium} high=${report.summary.high}（安全/低危/中危/高危）`);
 ...
 * @param config - `{ roots?, monitor?, hashOfficial?, gate?, allowlistTools? }`.
 */
function apply(ctx, config) {
	...
	// plugin-tool gate (default off): snapshot built-in tool names, then deny
	// calls to tools registered AFTER install unless allowlisted.
	const gateMode = config?.gate ?? "off";
	if (gateMode !== "off") {
		const tools = ctx.tools;
		const trusted = new Set((tools.schemas() ?? []).map((s) => s.name));
		trusted.add("plugin_vet"); // this plugin's own tool
		for (const t of config?.allowlistTools ?? []) trusted.add(t);
		ctx.effect(() => tools.guard((execution) => gateDecision(execution.name, trusted, gateMode)), "plugin-vet: tool gate");
	}
						...
						items: {
							...
							properties: {
								...
								deps: {
									...
									additionalProperties: false,
									properties: {
										ownScripts: { type: "array", items: { type: "string" } },
